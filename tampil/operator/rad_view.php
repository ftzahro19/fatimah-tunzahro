...
if ($_GET['action'] == "del")
{
   ...
   for ($i=0; $i<=$n-1; $i++)
   {
     if (isset($_POST['kd_rad'.$i]))
     {
       $kd_rad = $_POST['kd_rad'.$i];
       $nama_rad = $_POST['nama_rad'.$i];
       $query = "DELETE FROM rad WHERE id='$kd_rad'";
       mysql_query($query);
     }
   }
}
...
<body id="tab3">
<a href="rad.php?kd_kunjungan=<?php echo $kd_kunjungan;?>" onClick="NewWindow(this.href,'name','800','400','yes');return false"><b>+ Order rad </b>
</a>
...
<table align="center" width="100%" border="0" cellpadding="2" cellspacing="1" bgcolor="#DBEAF5">
 <tr bgcolor="#FFFFFF">
    ...
    <td width="23%" bgcolor="#D9E8F3"><div align="center">Nama rad</div></td>
...
?>
  <tr onMouseOver="this.bgColor='lightyellow'" onMouseOut="this.bgColor='#ffffff'" bgcolor="#ffffff">
    ...
    <input type="checkbox" name="kd_rad<?php echo $i;?>" value="<?php echo $data['id'];?>" />
