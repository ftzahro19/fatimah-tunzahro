...
while ($data = mysql_fetch_array($hasil))
{
   ...
   // menampilkan status 
   xlsWriteLabel($noBarisCell,5,$data['SenderID']);
...
while ($data = mysql_fetch_array($hasil))
{
   ...
   xlsWriteLabel($noBarisCell,5,$tgllhr);
...
else if ($_GET['op'] == "inboxfolder")
{

$idfolder = $_GET['idfolder'];

$query = "SELECT folder FROM sms_folder WHERE id = '$idfolder'";
$hasil = mysql_query($query);
$data  = mysql_fetch_array($hasil);
$folder = str_replace(" ", "-", $data['folder']);

$namaFile = "report-inbox-".$folder."-".$tgl.".xls";
...
$query = "SELECT * FROM sms_inbox WHERE idfolder = '$idfolder' ORDER BY id";
$hasil = mysql_query($query);
